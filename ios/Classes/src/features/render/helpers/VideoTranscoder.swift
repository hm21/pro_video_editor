...
///
/// This is necessary because GPU-based effect pipelines on iOS have compatibility
...
internal class VideoTranscoder {
    ...
    static func needsTranscoding(_ videoPath: String) async -> Bool {
        ...
        print("🔍 Video transcoding check: path=\(videoPath), " +
    ...
    static func transcodeToH264(_ videoPath: String) async -> TranscodeResult {
        ...
        do {
            ...
            print("   Output: isHevc=\(outputInfo.isHevc), bitDepth=\(outputInfo.bitDepth), isHdr=\(outputInfo.isHdr)")
    ...
    private static func transcodeVideo(from inputURL: URL, to outputURL: URL) async throws {
        ...
        guard let exportSession = AVAssetExportSession(asset: asset, presetName: AVAssetExportPresetHighestQuality) else {
            throw NSError(domain: "VideoTranscoder", code: 1,
        ...
        if #available(iOS 15.0, *) {
            ...
            guard let vTrack = videoTracks.first else {
                throw NSError(domain: "VideoTranscoder", code: 2,
        ...
        let renderSize = calculateOutputSize(naturalSize: naturalSize, transform: preferredTransform)
        
        // Create video composition with SDR color space filter
        let videoComposition = AVMutableVideoComposition(asset: asset) { request in
            var image = request.sourceImage.clampedToExtent()
            
            // Apply color space conversion (HDR → SDR happens automatically when rendering to sRGB context)
            // Use a simple color matrix to ensure SDR output
            if let colorMatrix = CIFilter(name: "CIColorMatrix") {
                colorMatrix.setValue(image, forKey: kCIInputImageKey)
                // Identity matrix - the HDR → SDR conversion happens via the render context
        ...
        // Force SDR rendering by using Rec. 709 color space
        ...
        guard exportSession.status == .completed else {
            ...
            throw NSError(domain: "VideoTranscoder", code: 3,
    ...
    private static func calculateOutputSize(naturalSize: CGSize, transform: CGAffineTransform) -> CGSize {
