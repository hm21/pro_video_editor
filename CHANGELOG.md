## 2.11.2
- **PERF**(android, iOS, macOS): A stop-motion frame that lives on disk is handed to the renderer as a path instead of being copied into the render call. The whole sequence used to cross the platform channel as bytes in one message — a few hundred phone photos at 3–4 MB each is past Android's managed-heap limit before the first one is decoded, and the export died with an `OutOfMemoryError`. Frames are now opened one at a time as they are encoded. Build a `StopMotionFrame` from `EditorLayerImage.file` to get this; an in-memory, asset or network source still travels as bytes.- **PERF**(android): A `VideoRenderData.audioTracks` entry is pre-rendered through a scratch file instead of one PCM byte array. Decoding a three-minute stereo track produced ~30 MB in the managed heap, plus a copy of it every time the array grew — on the same export that is already holding the video pipeline.
