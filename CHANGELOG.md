...
- **PERF**(android): Converting an overlay's premultiplied alpha no longer allocates a second full-frame buffer. It read the pixels out of the direct `ByteBuffer` into a `ByteArray` of the same size and wrote them back; both are Java-heap allocations against the same 256 MiB per-app limit, once per layer. The conversion now works through a 64 KiB scratch band, so the peak drops without the arithmetic getting slower.
