## 2.11.2
- **PERF**(android, iOS, macOS): A stop-motion frame that lives on disk is handed to the renderer as a path instead of being copied into the render call. The whole sequence used to cross the platform channel as bytes in one message — a few hundred phone photos at 3–4 MB each is past Android's managed-heap limit before the first one is decoded, and the export died with an `OutOfMemoryError`. Frames are now opened one at a time as they are encoded. Build a `StopMotionFrame` from `EditorLayerImage.file` to get this; an in-memory, asset or network source still travels as bytes.
## 2.11.1
