## 1.19.0
- **FEAT**(android, iOS, macOS): Add `VideoRenderData.audioLoopCrossfadeMillis` for seamlessly looping custom audio tracks. When set above `0` (default `0`, disabled), an equal-power crossfade of the given length is baked into the pre-rendered PCM at the loop seam of each looping track (`loop == true`): the first few milliseconds of the looped body are blended with the loop continuation — the real, consecutive source samples that would follow it — so the rendered file's last sample connects continuously to its first in both value and slope. This removes the click/"blob" heard when a rendered video is played back on repeat, even when the source audio is not itself a clean loop. Equal-power weighting keeps the loudness constant, so there is no audible volume dip.
