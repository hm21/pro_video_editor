## 2.11.3
- **FIX**(iOS, macOS): A render or split cancelled *before* its export actually started no longer terminates the app. The job registered its `AVAssetExportSession` for cancellation before waiting on the shared encoder gate, so a cancel arriving during that wait — opening the editor and leaving within a few seconds was enough — force-cancelled a session that had never run. AVFoundation counts `.cancelled` as started, so when the gate then let the job through, `export(to:as:)` assigned `outputURL` on it and raised `NSInternalInconsistencyException` ("Cannot alter output URL attribute on an AVAssetExportSession after an export has started") — an Objective-C exception Swift cannot catch from an async context, so the host app was killed rather than the error surfacing. An unstarted session is now left alone and the refused start surfaces as an ordinary cancellation. The same window existed on the bitrate-cap passthrough fast path, which attaches its session before awaiting a track load.
