## 2.13.0
- **FEAT**(android, iOS, macOS): A failed render, split, audio extraction or merge now says what failed in a form an app can branch on. Its `PlatformException.details` carry the platform's error domain and code plus the chain of causes underneath, read through the new `NativeFailureDetails.of(error)`; `isOutOfStorage` names the one failure a retry cannot fix. The message stays the platform's own description — which AVFoundation localises and Media3 reduces to one line per error code, dropping the exception that actually broke.
- **FIX**(iOS, macOS): A cancelled job answers its call before the cancel does, and its id is free from that moment. The export session unwinds asynchronously, and only its completion used to release the id, so a caller that cancelled and immediately restarted the same id — a retry after a stalled export — was refused with `TASK_ALREADY_RUNNING` while the old session was still tearing down. Same contract as Android.
## 2.12.1
