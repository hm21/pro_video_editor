## 2.6.0
- **FIX**(iOS, macOS): Concurrent split and render re-encodes no longer contend for the hardware encoder. They now share a process-wide gate so only one runs at a time; previously they starved each other on the shared VideoToolbox pool — measured, three concurrent 4K re-encodes each took ~43s versus ~19s alone — and on a throttled device a starved session could sit at 0% until the 120s watchdog (the reported split freeze). Each encode now runs at its solo speed with steady progress, and the wait is queued *before* the watchdog starts, so it never counts as a stall. Passthrough/stream-copy exports (no encoder) stay ungated.
- **FEAT**(android, iOS, macOS): A stalled clip split now fails fast with a diagnostic reason instead of hanging for the full 120s watchdog. If an export makes no forward progress for `SplitVideoModel.stallTimeout` (default 12s) it is force-cancelled, and both the stall and the hard-timeout errors now carry context — e.g. `Split export stalled after 12s with no progress [half=end progress=0.00 segment=0.52s split=0.53s total=1.05s preset=… audio=false]` (`preset` on iOS/macOS, `mime` on Android) — surfaced identically on both platforms. `exportTimeout` (hard bound) and `stallTimeout` (inner bound) are now tunable on `SplitVideoModel`; successful splits are unaffected.
