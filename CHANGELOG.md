## 2.13.0
- **FEAT**(android, iOS, macOS, web): Add `ProVideoEditor.getThumbnailStream` — the thumbnails of a `ThumbnailConfigs` request delivered as a stream of `ThumbnailFrame`s while they are being decoded, instead of once the whole set is ready. The whole request is one native decode pass, where a caller that wanted progressive fill from `getThumbnails` had to split it into small batches and pay a fresh decode pass from the keyframe for each one; on a timeline strip that decoded the clip once per batch. Frames arrive in decode order and carry the `indices` into `timestamps` they resolve to. Cancelling the subscription (or `cancel(id)`) stops the native decoder. Web delivers the complete set frame by frame.
- **FEAT**(android): `ThumbnailConfigs.maxParallelDecoders` caps the hardware decoder sessions the timestamp path runs at once (default 3). A timeline strip that shares the device's decoder pool with a live preview player passes `1` so it never holds more than one session and cannot starve playback.
- **FIX**(android): When the single-pass decoder fails partway through a `getThumbnails` request, the fallback extractors now decode only the frames that are still missing instead of the whole request again, and frames the single pass could not produce are no longer silently dropped.
