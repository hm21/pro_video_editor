## 1.21.5
- **FIX**(android): Fix video export failing with a codec exception on Qualcomm `c2.qti.*` encoders (e.g. trimming H.264 High-profile 1080p). Media3 requests `operating-rate = Integer.MAX_VALUE`, which these encoders reject at `configure()`/`start()`. The encoder is now retried through a fallback chain — capped operating-rate → operating-rate unset → Main/Baseline profile → software encoder — before surfacing a typed `RenderEncoderException`. The fast default is still tried first, so devices that already worked keep their original export speed, and the slow software encoder is only the last resort once every hardware option has failed.
- **FIX**(android): Fix a render/cancel race where a cancel issued right after starting an export returned `TASK_NOT_FOUND` because the native task wasn't registered yet. A cancel that arrives first is now a safe no-op and the pending render resolves as canceled instead of running un-cancellable.
